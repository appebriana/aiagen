<x-aiagen-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-secondary-200">
            <div class="flex flex-col md:flex-row md:items-center gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-sm text-secondary-500 mt-1">Pilih pengguna untuk melihat laporan interaksi Livechat miliknya.</p>
                </div>
                <form method="GET" action="{{ route($routePrefix . '.laporan.interaksi.livechat') }}" class="flex items-center gap-3">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-secondary-200">
            <div>
                <h2 class="text-xl font-bold text-secondary-900">Statistik Interaksi Livechat</h2>
                <p class="text-sm text-secondary-500">Pantau performa AI dalam melayani pesan pengunjung via widget Livechat.</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative inline-block text-left" x-data="{ isRangeOpen: false }">
                    ...
                    <div x-show="isRangeOpen" @click.away="isRangeOpen = false" x-cloak class="origin-top-right absolute right-0 mt-2 w-48 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-20 overflow-hidden">
                        <div class="py-1">
                            ...
                            @endphp
                            <a href="{{ route($routePrefix . '.laporan.interaksi.livechat', array_merge(['range' => 'harian'], $baseParams)) }}" class="block px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 {{ $range === 'harian' ? 'bg-primary-50 text-primary-700 font-bold' : '' }}">Harian</a>
                            <a href="{{ route($routePrefix . '.laporan.interaksi.livechat', array_merge(['range' => 'mingguan'], $baseParams)) }}" class="block px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 {{ $range === 'mingguan' ? 'bg-primary-50 text-primary-700 font-bold' : '' }}">Mingguan</a>
                            <a href="{{ route($routePrefix . '.laporan.interaksi.livechat', array_merge(['range' => 'bulanan'], $baseParams)) }}" class="block px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 {{ $range === 'bulanan' ? 'bg-primary-50 text-primary-700 font-bold' : '' }}">Bulanan</a>
                            <a href="{{ route($routePrefix . '.laporan.interaksi.livechat', array_merge(['range' => 'tahunan'], $baseParams)) }}" class="block px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 {{ $range === 'tahunan' ? 'bg-primary-50 text-primary-700 font-bold' : '' }}">Tahunan</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 ml-2 pl-4 border-l border-secondary-200">
                    <a href="{{ route($routePrefix . '.laporan.interaksi.livechat.export.excel', array_merge(['range' => $range], $baseParams ?? [])) }}" 
                    ...
                    <a href="{{ route($routePrefix . '.laporan.interaksi.livechat.export.pdf', array_merge(['range' => $range], $baseParams ?? [])) }}" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-secondary-200 overflow-hidden">
            <div class="p-6 border-b border-secondary-100 flex items-center justify-between">
                <h3 class="font-bold text-secondary-900 flex items-center gap-2">
                    ...
                    Daftar Pengunjung Teraktif (Top 10)
                </h3>
            </div>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-secondary-100">
                        ...
                        <tr class="hover:bg-secondary-50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 bg-secondary-100 rounded-md text-xs font-mono text-secondary-600" title="{{ $item->customer_phone }}">{{ Str::limit($item->customer_phone, 15) }}</span>
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="9" class="px-6 py-12 text-center text-secondary-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>Belum ada data interaksi Livechat yang tercatat.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="flex items-center justify-center min-h-[40vh]">
            <div class="text-center max-w-md mx-auto">
                ...
                <p class="text-sm text-secondary-500">Silakan pilih akun pengguna di atas untuk melihat laporan interaksi Livechat miliknya.</p>
            </div>
        </div>
        @endif
    </div>
    ...
    {{-- MODAL DETAIL --}}
    @include('pengguna.laporan._modal_detail_livechat')
