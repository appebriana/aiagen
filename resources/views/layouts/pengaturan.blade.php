                ...
                <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 scrollbar-hide">
                    ...
                    <a href="{{ auth()->user()->isAdmin() ? route('admin.pengaturan.index') : route('pengguna.pengaturan.index') }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                        ...
                        <span x-show="sidebarOpen" x-cloak>Pengaturan Utama</span>
                    </a>
                ...
                <div class="bg-white/90 backdrop-blur-2xl rounded-2xl shadow-[10px_10px_40px_rgba(0,0,0,0.12)] border border-white/50 flex flex-col h-full">
                    ...
                    <div class="p-5 overflow-y-auto flex-1 scrollbar-hide">
                            ...
                            <div class="flex flex-col gap-2 mb-6">
                                <p class="px-3 text-[10px] font-bold text-primary-500 uppercase tracking-widest mb-1">Navigasi Utama</p>
                                
                                <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('pengguna.dashboard') }}"
                                   class="flex items-center gap-4 p-3 rounded-2xl transition-all duration-300 active:scale-95 bg-secondary-50 text-secondary-600 hover:bg-secondary-100">
                                ...
                                <div x-data="{ isLaporanOpen: false }">
                                    <button @click="isLaporanOpen = !isLaporanOpen"
                                            class="w-full flex items-center justify-between p-3 rounded-2xl transition-all duration-300 active:scale-95 bg-secondary-50 text-secondary-600 hover:bg-secondary-100">
                                    ...
                                    <div x-show="isLaporanOpen" x-transition class="mt-2 ml-4 pl-4 border-l-2 border-primary-100 space-y-1">
                                        <a href="{{ auth()->user()->isAdmin() ? route('admin.laporan.interaksi.wa') : route('pengguna.laporan.interaksi.wa') }}"
                        ...
                        {{-- Vertical Menu Items --}}
                        <div class="flex flex-col gap-2 mb-6">
                            <p class="px-3 text-[10px] font-bold text-primary-500 uppercase tracking-widest mb-1">Menu Pengaturan</p>
                            
                            <a href="{{ auth()->user()->isAdmin() ? route('admin.pengaturan.index') : route('pengguna.pengaturan.index') }}" 
                               class="flex items-center gap-4 p-3 rounded-2xl transition-all duration-300 active:scale-95 {{ request()->routeIs('*.pengaturan.index') ? 'bg-primary-600 text-white shadow-lg shadow-primary-200' : 'bg-secondary-50 text-secondary-600 hover:bg-secondary-100' }}">
                                <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                <span class="font-bold text-sm">Pengaturan Utama</span>
                            </a>

                            <a href="{{ auth()->user()->isAdmin() ? route('admin.profile.edit') : route('pengguna.profile.edit') }}" 
                               class="flex items-center gap-4 p-3 rounded-2xl transition-all duration-300 active:scale-95 {{ request()->routeIs('*.profile.edit') ? 'bg-primary-600 text-white shadow-lg shadow-primary-200' : 'bg-secondary-50 text-secondary-600 hover:bg-secondary-100' }}">
                            ...
                            @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.users.index') }}" 
                               class="flex items-center gap-4 p-3 rounded-2xl transition-all duration-300 active:scale-95 {{ request()->routeIs('admin.users.index') ? 'bg-primary-600 text-white shadow-lg shadow-primary-200' : 'bg-secondary-50 text-secondary-600 hover:bg-secondary-100' }}">
                                <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
            ...
            <div class="flex-1 flex flex-col transition-all duration-300 min-w-0" :class="sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'">
                ...
                <header class="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-secondary-300 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-sm">
                    ...
                    <div class="hidden lg:flex items-center gap-2">
                        ...
                        <a href="{{ auth()->user()->isAdmin() ? route('admin.ai-agen.index') : route('pengguna.ai-agen.index') }}" 
                           class="px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2 {{ (request()->is('*/ai-agen*') || request()->is('*/laporan*')) && !request()->is('*/cms*') ? 'bg-primary-100 text-primary-700' : 'text-secondary-600 hover:bg-primary-100' }}">
